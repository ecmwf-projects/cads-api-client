...
class ApiResponse:
    ...
    def get_link_href(self, **kwargs: str) -> str:
        ...
        if len(links) != 1:
            raise LinkError(f"link not found or not unique {kwargs}")
        return links[0]["href"]
...
class Remote:
    ...
    def robust(self, call: Callable[..., Any]) -> Callable[..., Any]:
        robust: Callable[..., Any] = multiurl.robust(call, **self.retry_options)
        return robust
